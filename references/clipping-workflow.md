...
Use `python3` and FFmpeg/FFprobe on PATH. If this installed skill contains
`runtime.json`, read its `python` and `bin_dir` values and use those absolute
paths (prepend `bin_dir` to PATH). Run preflight in that same environment.
Do not assume the editor's embedded FFmpeg is usable as a standalone binary.

## Select the operating mode

- **CPU + visual review (default for NAS footage or no trained model):** Read
  [references/cpu-editor-workflow.md](cpu-editor-workflow.md). Create
  local review proxies, inspect continuous action, record source timestamps,
  and use Jianying/剪映 when requested and desktop control is available. This
  is assisted clipping, not a claim of unattended recognition of every basket.
...
Read [references/detection-modes.md](detection-modes.md) when choosing or implementing a detector. Read [references/event-schema.md](event-schema.md) before generating or editing event files.
...
6. Apply confidence routing:
   - Explicit visual acceptance (`result: made`, `review.status: accepted`) takes precedence over detector confidence; preserve the original confidence. A rejected review is never exported.
   ...
   - Fixed-camera baseline output is always review-first, regardless of its numeric confidence. Set both `result` and `review.status` after review.
